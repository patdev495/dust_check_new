    ...
    def dust_detect_on_image(self, image: str | np.ndarray) -> None | tuple[np.ndarray,np.ndarray,bool,np.int32]:
        ...
        if cropped_image is not None:
            ...
            if abnormal_result is not None:
                ...
                anomaly_map = cv2.resize(abnormal_result['anomaly_map'], (int(width), int(height)))
                max_value_abnormal = np.max(anomaly_map)  
                logger.debug(f"Max value in heatmap: {max_value_abnormal}")
                binary_heatmap = self.normalize_heatmap_to_binary(anomaly_map)
                ...
                if len(contours) == 0:
                    ...
                    for contour in contours:
                        ...
                        heatmap_values = anomaly_map[mask == 255]
