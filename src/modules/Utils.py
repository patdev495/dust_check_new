...
class StableFocusScore:
    def __init__(self, buffer_size=5, blur_ksize=(3, 3)):
        """
        buffer_size: số lượng điểm để tính trung bình mượt.
        blur_ksize: kernel kích thước làm mờ Gaussian.
        """
    ...
    def calculate(self, image):
        """
        Tính focus score ổn định từ ảnh đầu vào.
        """
        ...
        # Làm mờ nhẹ để giảm nhiễu rung nhỏ
        ...
        # Cập nhật bộ đệm
        self.buffer.append(variance)

        # Trả về giá trị trung bình mượt
...
def calculate_brightness_score(image):
    """
    Tính độ sáng trung bình của ảnh BGR.
    
    Args:
        image: Ảnh đầu vào dạng numpy array (BGR).

    Returns:
        float: Giá trị độ sáng trung bình từ 0 đến 255.
    """
    gray: np.ndarray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    brightness: float = float(np.mean(gray))
    return brightness

def draw_focus_and_brightness_score(frame,focus_score=None, brightness_score=None):
    if focus_score is None:
        focus_score = calculate_focus_score(frame)
    if brightness_score is None:
        brightness_score = calculate_brightness_score(frame)
    text = f"Focus: {int(focus_score)}, Brightness: {int(brightness_score)}"
