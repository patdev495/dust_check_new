    ...
    def dust_detect_on_image(self, image: str | np.ndarray) -> None | tuple[np.ndarray,np.ndarray,bool,np.int32]:
        ...
        if cropped_image is not None:
            ...
            if abnormal_result is not None:
                ...
                if abnormal_inference_params.is_need_to_dilated:
                    kernel = np.ones((abnormal_inference_params.dilated_kernel_size, abnormal_inference_params.dilated_kernel_size), dtype=np.uint8)
                    dilated_heatmap = cv2.dilate(binary_heatmap, kernel, iterations=1)
                else:
                    dilated_heatmap = binary_heatmap
