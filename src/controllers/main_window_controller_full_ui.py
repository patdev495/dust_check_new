...
from src.modules.Utils import (
    ...
    draw_focus_and_brightness_score,
    StableFocusScore
)
import pickle
...
from dump_pickle import dump_pickle_products
...
class StreamVideoWorker(QObject):
    ...
    def __init__(self, parent):
        ...
        self.calculator_focus = StableFocusScore(buffer_size=system_config_params.buffer_size_calculate_focus,blur_ksize=system_config_params.kernel_blur_size_calculate_focus)   
        ...
        while self.is_running and self.cap is not None and self.cap.isOpened():
            ...
            if not self._parent.is_auto_process:
                if system_config_params.show_image_focus_score:
                    if self.frame_count % system_config_params.skip_frames_show_focus == 0:
                        current_focus = math.floor(self.calculator_focus.calculate(frame))
                        current_brightness = math.floor(calculate_brightness_score(frame))
...
class MainWindowController(QMainWindow, Ui_MainWindow):
    ...
    def load_products_data(self) -> None:
        if not os.path.exists('data_products.pkl'):
            dump_pickle_products()
            
        with open('data_products.pkl', 'rb') as f:
            self.data_products = pickle.load(f)
            
        now = datetime.now().time()
        start = tm(timer_config_params.day_start_time[0], timer_config_params.day_start_time[1])  # 7:30 sáng
        end = tm(timer_config_params.day_end_time[0], timer_config_params.day_end_time[1])  # 19:30 chiều
        if start < now < end:
            # if datetime.now().date() != self.data_products['last_day_reset']:
            #     dump_pickle_products()
            if not self.data_products['day_reset']:
                self.data_products['sum_ok'] = 0
                self.data_products['day_reset'] = True
                self.data_products['night_reset'] = False
                # self.data_products['last_day_reset'] = datetime.now().date()     
        else:
            # if datetime.now().date() != self.data_products['last_day_reset']:
            #     dump_pickle_products()
            if not self.data_products['night_reset']:
                self.data_products['sum_ok'] = 0
                self.data_products['night_reset'] = True
                self.data_products['day_reset'] = False
                # self.data_products['last_day_reset'] = datetime.now().date()
    
    def dump_pickle_products_data(self) -> None:
        with open('data_products.pkl', 'wb') as f:
            pickle.dump(self.data_products, f)
    ...
    def init_variables(self) -> None:
        ...
        self.ui_sum_ok_label.setText(f"N OK: {self.data_products['sum_ok']}")
        ...
        if self.stream_video_thread is not None:
            try:
                ...
                if not self.stream_video_thread.wait(5000):
                    console_logger.warning("Cannot stop thread, force terminate")
            ...
            try:
                ...
                if (
                    ...
                    try:
                        ...
                        self.ui_sum_ok_label.setText(f"N OK: {self.data_products['sum_ok']}")
                        self.dump_pickle_products_data()
                        ...
                        self.write_log_to_txt(self.input_mac_value)
    ...
    # Đảm bảo thư mục log tồn tại
        ...
        # Tạo tên file dạng dd-mm-yyyy.txt
        ...
        # Tạo dòng log: Scan Time, SN, Cycle time (giữa các phần tử cách nhau tab hoặc dấu phẩy)
        cycle_time = getattr(self, "cycle_time", "N/A")
        log_line = f"{current_time}\t{log_text}\t{cycle_time}"

        try:
            # Mở file ở chế độ append, ghi thêm dòng log
            with open(file_path, "a", encoding="utf-8") as f:
                # Nếu file mới vừa tạo, bạn có thể thêm dòng header
                if os.path.getsize(file_path) == 0:
                    f.write("Scan Time\tSN\tCycle time\n")
                f.write(log_line + "\n")
        except Exception as e:
            print(f"Cannot write log to  txt: {e}")
    ...
    def save_pass_image(
        ...
        print(f"✅ Save image succesfully: {filename}")
